...
class NmeaTestCase(unittest.TestCase):
    def test_01_location(self):
        ...
        self.assertTrue(isinstance(loc, location.Location))
    ...
    def test_02_validate_nmea_checksum(self):
        ...
        bool1, sen1 = location.validate_nmea_checksum(good)
        bool2, sen2 = location.validate_nmea_checksum(bad)
        del sen2   #: unused
    ...
    def test_03_get_location(self):
        ...
        loc = location.location_from_nmea(nmea_data)
        match = True
        for v in vars(loc):
            if isinstance(vars(loc)[v], list):
                ...
                match = False
                break
        self.assertTrue(isinstance(loc, location.Location) and match)
