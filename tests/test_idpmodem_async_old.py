...
from idpmodem.helpers import validate_serial_port
...
def modem(mocker: MockerFixture):
    ...
    #     mocker.patch('idpmodem.helpers.validate_serial_port',
    #         return_value=(True, 'mockSerial'))
    return IdpModemAsyncioClient(port=SERIAL_PORT, log_level=LOG_LEVEL)
...
def mock_command_ok(monkeypatch):
    mock = CoroutineMock(
        idpmodem.asyncio.atcommand_async.IdpModemAsyncioClient.command,
        return_value=['OK']
    )
    monkeypatch.setattr(idpmodem.asyncio.atcommand_async.IdpModemAsyncioClient,
        'command', mock)
    return mock
